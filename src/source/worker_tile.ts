...
function detectSdfPatterns(bucket: FillBucket | LineBucket, patternMap: GetImagesResponse): boolean {
    let sdfPatterns: boolean | undefined;
    const check = (isSdf: boolean) => {
        if (sdfPatterns === undefined) {
            sdfPatterns = isSdf;
        } else if (sdfPatterns !== isSdf) {
            warnOnce('Cannot mix SDF and non-SDF patterns in one layer');
        }
    };
    ...
    // Constant patterns
    const paintPropName = bucket instanceof LineBucket ? 'line-pattern' : 'fill-pattern';
    for (const layer of bucket.layers) {
        const constantPattern = (layer as any).paint.get(paintPropName).constantOr(null);
        if (constantPattern) {
